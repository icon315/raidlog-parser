...
string my_name = my_name().to_lower_case();
...
string bossName(string s) {
	...
	case "Parts": return "Richard";
	case "Woods":
		if(data[".data","DreadWoods"] contains "!Werewolf") return "The Great Wolf of the Air";
		else if(data[".data","DreadWoods"] contains "!Bugbear") return "Falls-From-Sky";
		else return "Unknown";
	case "Village":
		if(data[".data","DreadVillage"] contains "!Ghost") return "Mayor Ghost";
		else if(data[".data","DreadVillage"] contains "!Zombie") return "Zombie HOA";
		else return "Unknown";
	case "Castle":
		if(data[".data","DreadCastle"] contains "!Vampire") return "Count Drunkula";
...
string getName(string l) {
	...
	if(!m.find()) return ".";
	data[".data",".id",m.group(1).to_lower_case()] = m.group(2).to_int();
	return m.group(1).to_lower_case();
}
	...
	foreach area,locNum in theMatcher["DreadPencil"] {
		...
		if(data[".data",".pencil",nc] == 1) {
			...
			foreach e in $strings[Hot,Cold,Spooky,Sleaze,Stench] if((resultList contains ("-"+e))&&(data[".data",zone,"."+e] == 0)) {
				...
				if((zone == 'DreadVillage')&&(e == 'Hot')&&(data[".data",zone,".Cold"] == 0))
					contexts[zone+"_"+e] += ", callback: function(key, options) {if(confirm('Cold has not been banished yet, banishing Hot now will make this impossible, do you wish to continue?')) {window.location='clan_raidlogs.ash?cmd=g621';}}";
				contexts[zone+"_"+e] += '},';
				if(keyOp)
					contexts[zone+"_"+e] += '"toggle": {name: "Permit Dreadsylvania Key", callback: function() {this.data(\''+nc.noSpaces()+'Disabled\', !this.data(\''+nc.noSpaces()+'Disabled\')); return false;}}';
				contexts[menu] += '},';
			}
...
void formatHB(string show) {
	...
	foreach index in odata {//Full Area Table Data
		write("<tr class=\"rowD"+(odata[index].pullField(".name").to_lower_case() == my_name?" HL":"")+"\"><td class=\"TSO\" style=\"text-align:left\">"+linkify(odata[index].pullField(".name"))+"</td>");
	...
	foreach area in $strings[Sewer,TS,BB,EE,Heap,AHBG,PLD] {//Per-area tables
		...
		foreach index in odata {//per-area table data
			write("<tr class=\"rowD"+(odata[index].pullField(".name").to_lower_case() == my_name?" HL":"")+"\"><td class=\""+area+"O\" style=\"text-align:left\">");
...
void formatST(string show) {
	...
	foreach index in odata {//slime table rows
		write("<tr class=\"rowD"+(odata[index].pullField(".name").to_lower_case() == my_name?" HL":"")+"\"><td class=\"SlimeO\" style=\"text-align:left\">"+linkify(odata[index].pullField(".name"))+"</td>");
...
/*	switch(data[".htotal","Haunted","!Necbromancer"]) {//Write the boss table text
	...
	foreach index in odata {//haunted table rows
		...
		write("<tr class=\"rowD"+(odata[index].pullField(".name").to_lower_case() == my_name?" HL":"")+"\"><td class=\"HauntedO\" style=\"text-align:left\">"+linkify(odata[index].pullField(".name"))+(odata[index,"Haunted","!Necbromancer"]>0?" ("+odata[index,"Haunted","!Necbromancer"]+")":"")+(odata[index,"Haunted","!Guides"]>0?" (G)":"")+"</td>");
...
void formatDreadNCTable() {
	...
	foreach i in layout{
		...
		case 0:
			...
			break;
		case 1:
			zone = "DreadWoods"; write("<tr class=\"RowD\">");
			break;
		case 2:
			zone = "DreadVillage";
			break;
...
void formatDread(string show) {
	...
	foreach index in odata {
		write("<tr class=\"rowD"+(odata[index].pullField(".name").to_lower_case() == my_name?" HL":"")+"\"><td class=\"TSO\" style=\"text-align:left\">");
...
//  write("</td></tr><tr><td colspan=\"3\"><center><div id=\""+area+"Table\" style=\"display:inline;\"><table class=\"sortable tableD\"><tr>");
		...
		foreach index in odata {
			...
			foreach i,s in specs
				foreach e in $strings[Hot, Cold, Spooky, Sleaze, Stench, All]
					if(exists[s,e])
...
void conMan3() {
	...
	foreach name,amount in FF if(name.substring(0,2) == "u.") {
		...
		switch(split[2]) {
		case "Sliders": stuff[split[1],$item[extra-greasy slider]] = amount.to_int(); break;
		case "Pickle Juice": stuff[split[1],$item[jar of fermented pickle juice]] = amount.to_int(); break;
		case "Banquets": stuff[split[1],$item[frozen banquet]] = amount.to_int(); break;
		case "Blankets": stuff[split[1],$item[Hodgman's blanket]] = amount.to_int(); break;
		case "Hodge Stew": stuff[split[1],$item[tin cup of mulligan stew]] = amount.to_int(); break;
		case "Forks": stuff[split[1],$item[Ol' Scratch's salad fork]] = amount.to_int(); break;
		case "Mugs": stuff[split[1],$item[Frosty's frosty mug]] = amount.to_int(); break;
		case "Snuff": stuff[split[1],$item[voodoo snuff]] = amount.to_int(); break;
		}
	}
